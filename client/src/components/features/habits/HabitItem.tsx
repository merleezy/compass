...
function StreakBadge({ streak, completedToday }: { streak: number; completedToday: boolean }) {
...
export default function HabitItem({ habit, onToggle, onDelete, onEdit }: HabitItemProps) {
  ...
  // The generic tells useRef what element the ref will hold, so
  // menuRef.current is a typed HTMLDivElement (not just "anything") below
