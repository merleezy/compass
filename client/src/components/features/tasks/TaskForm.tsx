...
interface TaskFormProps {
  onCreate: (title: string, description: string, dueDate: string | null, tags: string[]) => void;
  onClose: () => void;
  existingTags?: string[];
}

export default function TaskForm({ onCreate, onClose, existingTags = [] }: TaskFormProps) {
  const [title, setTitle] = useState("");
  const [description, setDescription] = useState("");
  const [dueDate, setDueDate] = useState("");
  // useState([]) alone infers never[] (an array nothing can go into), so the
  // generic spells out what the array will hold
