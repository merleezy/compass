...
export default function HabitsPage() {
  ...
  const fetchHabits = useCallback(async () => {
    ...
    try {
      ...
      // res.json() returns Promise<any> — the annotation is our contract
      // that this endpoint serves Habit objects
  ...
  const handleToggle = async (habitId: string, isCurrentlyCompleted: boolean) => {
  ...
  const handleEdit = async (habitId: string, name: string, description: string) => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/habits/${habitId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
